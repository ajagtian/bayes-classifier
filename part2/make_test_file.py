...
def	get_id_map(words_file_name):
		f = open(words_file_name, 'rU', errors = 'ignore')
		lines = f.readlines()
		id = int(lines[0][:-2])
		line1  = lines[1] 
...
def	read_testing_files(dir, words_file_name, test_file_name):
		filenames = sorted(os.listdir(dir))
		(id_map, id) = get_id_map(words_file_name)
		out_file_handle = open(test_file_name, 'w')
...
def main():
	args = sys.argv[1:]
	if len(args) != 3:
		print('usage: ./svm_train.py <testing_dir> <words_file> <test_file_name>')
		sys.exit()

	read_testing_files(args[0], args[1], args[2])
