...
def	get_id_map(words_file):
		f = open(words_file, 'rU', errors = 'ignore')
		line1 = f.readlines()[1] 
		id_map = ast.literal_eval(line1)
		return id_map


def	read_training_files(dir, neg_class, pos_class, words_file, training_file_name):
		filenames = sorted(os.listdir(dir))
		id_map = get_id_map(words_file)
		out_file_handle = open(training_file_name, 'w')
...
def main():
	args = sys.argv[1:]
	if(len(args) != 5):
		print('usage: ./svm_train.py <training_dir> <neg_class> <pos_class> <words_file> <training_file_name>')
		sys.exit()

	read_training_files(args[0], args[1], args[2], args[3], args[4])
